struct LazyMinSegmentTree {
	int sz;
	ll *tree, *lazy;

	LazyMinSegmentTree(int s) {
	...
	LazyMinSegmentTree(int s, ll* st) {
		sz = 1 << (int)ceil(log2(s));
		tree = new ll[2 * sz];
		lazy = new ll[sz];
		memset(tree, 0, 2 * sz * sizeof(ll));
	...
	ll query(int ind, int st, int e, int l, int r) {
		...
		propagate(ind);
		int mid = (st + e) / 2;
		if (l < mid && r > mid) return min(query(2 * ind, st, mid, l, mid), query(2 * ind + 1, mid, e, mid, r));
