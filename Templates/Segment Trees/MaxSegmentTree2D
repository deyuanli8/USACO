struct MaxSegmentTree2D {
	int sz;
	MaxSegmentTree *tree;

	MaxSegmentTree2D(int s, int t) {
	...
	MaxSegmentTree2D(int s, int s2, ll** st) {
		...
		F0Rd(ind, 1, sz) {
			FOR(i, s2) {
				t[i] = max(tree[2 * ind].tree[tree[2 * ind].sz + i], tree[2 * ind + 1].tree[tree[2 * ind + 1].sz + i]);
			}
			tree[ind] = MaxSegmentTree(s2, t);
		}
	}
	void add(int ind1, int ind2, ll x) {
		...
		for (ind1 /= 2; ind1 >= 1; ind1 /= 2) {
			tree[ind1].add(ind2, max(tree[2 * ind1].tree[ind2 + tree[ind1].sz], tree[2 * ind1 + 1].tree[ind2 + tree[ind1].sz]) - tree[ind1].tree[tree[ind1].sz + ind2]);
		}
	}
	ll query(int a, int b, int c, int d) { //max of rows [a,b], columns [c,d] inclusive
		...
		while (a <= b) {
			if (a % 2 == 1) s = max(s, tree[a++].query(c, d));
			if (b % 2 == 0) s = max(s, tree[b--].query(c, d));
			a /= 2; b /= 2;
		}
		return s;
	}
